...
هر OSD یک **فرآیند** است: checksum، فشرده‌سازی، replication به OSDهای دیگر، پاسخ به کلاینت. دیسک خام CPU نمی‌خواهد؛ فرآیند OSD می‌خواهد. رقم ~۱ GHz به‌ازای هر OSD تخمین دوره است تا وقتی ۱۲ دیسک روی یک سرور می‌گذارید، CPU تمام هسته‌ها را برای recovery همزمان خفه نکند.
...
در recovery، OSDها object را در حافظه نگه می‌دارند تا کپی گم‌شده را دوباره روی دیسک‌های زنده بسازند. RAM کم یعنی cache کوچک و بازسازی کندتر — و در بدترین حالت OSD با OOM می‌میرد. رقم ۲ GB «برای روز عادی» نیست؛ برای روزی است که یک نود از دست رفته و بقیه در حال backfill هستند.

## دو شبکه: Client و Cluster

اگر کلاینت و replication روی **یک** لینک باشند، روز recovery ترافیک بازسازی لینک را پر می‌کند و VMها timeout می‌گیرند. شبکهٔ Cluster همان لولهٔ داخلی OSD↔OSD است؛ شبکهٔ Client همان لولهٔ MON و RBD/CephFS/RGW. جدا کردنشان یعنی سیل recovery، I/O کاربر را خفه نکند.
