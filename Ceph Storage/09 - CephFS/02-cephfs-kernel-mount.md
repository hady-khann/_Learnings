...
| قطعه | معنی در این لاب |
|---|---|
| `192.168.1.15:6789` | آدرس MON و پورت پیش‌فرض v1 مانیتور (`6789`) — کلاینت از اینجا نقشهٔ کلاستر را می‌گیرد |
| `:/` | بعد از پورت، مسیر export داخل CephFS؛ `/` یعنی ریشهٔ کل filesystem |
| `name=fs` | شناسهٔ CephX بدون پیشوند `client.` — یعنی کاربر `client.fs`. نام filesystem (`cephfs`) اینجا نیست |
| `secret=` | همان کلید داخل keyring. کرنل فایل keyring را مثل CLI خودکار نمی‌خواند؛ باید کلید را بدهید تا مثل هر کلاینت CephX احراز هویت شود |
...
در چت کلاس به این مطلب اشاره شد:
