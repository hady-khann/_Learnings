...
دو cache متفاوت‌اند و واحدشان یکی نیست:

- `mds cache size` تعداد **inode** (فایل/دایرکتوری در حافظهٔ MDS) است، نه بایت. عدد بزرگ‌تر یعنی `ls` روی درخت شلوغ کمتر به دیسک metadata می‌رود.
- `rbd cache size` بایت روی **کلاینت** RBD است (اینجا ۶۴ MiB). نوشتن‌های کوچک را قبل از فرستادن به OSD جمع می‌کند.
...
`67108864` بایت یعنی **۶۴ MiB** کش کلاینت RBD. عدد MDS تعداد inode در cache است، نه بایت. این دو را با هم جمع نکنید و روی OSD به‌عنوان RAM نود حساب نکنید.
...
`size` تعداد کپی هدف است (مثلاً ۳ — پوشهٔ `06 - Pool`). `min_size` کف است: اگر replicaهای زنده از این عدد کمتر شوند، Ceph نوشتن را **تأیید نمی‌کند**. یعنی:

- `size=3` و `min_size=2`: یک OSD می‌میرد، هنوز می‌نویسید (دسترس‌پذیری).
- اگر `min_size` را ۱ بگذارید، با یک کپی هم acknowledge می‌شود — خطر از دست رفتن داده بالاتر است.

باید از `osd pool default size` کوچک‌تر باشد.

اسلاید ۵۳: در حالت degrade، حداقل چند replica باید زنده باشد تا نوشتن از کلاینت acknowledge شود. اگر به این کف نرسید، Ceph نوشتن را تأیید نمی‌کند.
...
Placement Group کار را بین OSDها پخش می‌کند (جزئیات در `06 - Pool`). خیلی کم → چند OSD شلوغ می‌شوند؛ خیلی زیاد → هر OSD RAM و CPU بیشتری برای نقشهٔ PG می‌دهد. فرمول اسلاید حدود ۱۰۰ PG به‌ازای هر OSD را هدف می‌گیرد تا از هر دو طرف فاصله بگیرید.
