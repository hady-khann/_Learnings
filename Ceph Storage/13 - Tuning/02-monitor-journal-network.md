...
FileStore (قدیمی) نوشتن را اول روی **journal** می‌گذاشت. BlueStore (پیش‌فرض از Luminous، از جمله Pacific این دوره) به‌جای آن WAL/DB دارد. پس `osd_journal_size` را روی کلاستر BlueStore در `ceph.conf` کپی نکنید؛ بی‌اثر یا گمراه‌کننده است. WAL روی SSD در پوشهٔ `11 - Hardware` است.
...
MTU اندازهٔ یک فریم اترنت است؛ پیش‌فرض ۱۵۰۰ بایت. **Jumbo** معمولاً ۹۰۰۰ است: هر بسته دادهٔ بیشتری می‌برد، تعداد interrupt کمتر، throughput replication بین OSD بهتر. شرط: **هاست و سوئیچ هر دو** همان عدد را داشته باشند. اگر یکی ۱۵۰۰ و دیگری ۹۰۰۰ باشد، فریم تکه می‌شود یا drop می‌شود و شبکه بدتر از قبل کار می‌کند.
